...
# df = pd.read_csv("rankings_with_norm.csv")
# current = df[df["rank_date"] == '2024-04-04']
# print(current)

# uefa = current[current["confederation"] == "UEFA"].sort_values(by="rank")
...
euro = pd.read_csv("euro_teams.csv")
copa = pd.read_csv("copa_teams.csv")
...
def outcome_prob(row):
    ...
    # After normalizing it with the win probs, the average value is 0.18. The number of ties in the last euros and copa was .25. We shifted down a bit to adjust for the larger tie odds
    # Equation: home_prob = a, away_prob = b, c = probability shift (=3), c(ab)/(abs(b-a)+1)
...
def sim_group_stage_match(team1, team2, matchups):
    ...
    tie_prob = match['tie_prob'].values[0]
...
def determine_euro_knockout(teams, group_points, team_info):
    ...
    for team in second_place:
        away_team = team[0]
        # if "Group A" in team:
        #     home_team = [team2 for team2 in second_place if "Group B" in team2][0][0]
        #     game_num = 8
        if "Group B" in team:
            ...
            game_num = 2
        # elif "Group D" in team:
        #     home_team = [team2 for team2 in second_place if "Group E" in team2][0][0]
        #     game_num = 4
...
def sim_knockout(bracket, matchup_probs):
    # For the pool, winning R016 is 4pts, winning QF is 6, SF is 8, and F is 10 (added on, not cumulative, so a winner would get 28pts)
...
def choose_teams():
    ...
    costs = pd.read_csv("team_costs.csv")
...
choose_teams()
# print(MATCH_COUNT, TIE_COUNT)

# equation: # 1/(10**(-dr/30)+1)

# current rankings prob


# past two years rankings prob



# historical lin reg rankings prob